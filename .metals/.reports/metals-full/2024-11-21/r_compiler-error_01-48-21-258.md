...
class tile(T: Int, N: Int, mempot_width: Int) extends Module {
    ...
    when(s1_valid && !pipe_stall){
        when((s1_addr_calc(6)(8,5) >= 9.U(4.W)) && (s1_addr_calc(6)(4,1) >= 6.U(4.W))) {
            ...
            s2_mempot_rd(@@) := io.rd_data_L2_S(0)

            io.rd_en_L2_S(1) := true.B
            io.rd_addr_L2_S(1) := s1_addr_calc(7)
            s2_mempot_rd := io.rd_data_L2_S(1)

            io.rd_en_L2_S(2) := true.B
            io.rd_addr_L2_S(2) := s1_addr_calc(8)
            s2_mempot_rd := io.rd_data_L2_S(2)

        } .elsewhen ((s1_addr_calc(i)(8,5) < 9.U(4.W)) && (s1_addr_calc(4,1) < 3.U(4.W))) {
            io.rd_en_L2_N(i) := true.B
            io.rd_addr_L2_N(i) := s1_addr_calc(i)
            s2_mempot_rd := io.rd_data_L2_N(i)
