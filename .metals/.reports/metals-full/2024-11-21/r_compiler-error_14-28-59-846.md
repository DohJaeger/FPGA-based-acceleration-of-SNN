...
class tile(T: Int, N: Int, mempot_width: Int) extends Module {
    ...
    when(io.thresh_en && !wrap){
        when(read_mem_counter < 9.U){     // for now tot_hor_blk = 9
            io.rd_en_L1_N := true.B

            for(i <- 0 until 3){
                io.rd_addr_L1_N(i) := read_mem_counter
                s1_thresh_local_mem_rd(i) := io.rd_data_L1_N(i)
            }

            for(i <- 0 until 9){
                io.rd_en_main_mempot(i) := true.B
                io.rd_addr_main_mempot(i) := read_mem_counter
                s1_thresh_main_mem_rd(i) := io.rd_data_main_mempot(i)
            }

        } .elsewhen(read_mem_counter >= 9.U){
            io.rd_en_L1_S := true.B

            for(i <- 0 until 3){
                io.rd_addr_L1_S(i) := read_mem_counter
        ...
        read_mem_counter := read_mem_counter + 1.U
        s1_thresh_valid := true.B
    }
        
    // stage - 2: add bias
    when(!wrap && s1_thresh_valid){
        for(i <- 0 until 3){
    ...
    when(!wrap && s2_thresh_valid){
        when(read_mem_counter == 19.U){     // total 19 blks: tot_hor_blk * tot_ver_blk
            if(@@)
        }
    }
