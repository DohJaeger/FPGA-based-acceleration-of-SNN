...
class conv_pipelined(depth: Int, width: Int, mempotSize: Int, memPixelWidth: Int) extends Module {
  ...
  // s1 --> addr calculation of neighbour
    ...
    when(validS2 && !stallPipeline) {
        ...
        // Perform saturation arithmetic to avoid overflow/underflow
...
object conv_pipelined extends App {
  val verilogString = chisel3.getVerilogString(new conv(256, 11, 48, 9))
  val verilogFile = new File("conv_pipelined.sv")
