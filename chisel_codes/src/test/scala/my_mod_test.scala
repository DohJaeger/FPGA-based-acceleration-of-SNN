...
// class MyTopTestWithSubmodule extends AnyFlatSpec with ChiselScalatestTester {
//   "my_top" should "allow peeking into my_calc submodule signals and test memory initialization" in {
//     test(new my_top(50, 8)) { dut =>
//       // Reset the DUT
//       dut.reset.poke(true.B)
//       dut.reset.poke(false.B)
//       dut.io.init.poke(true.B)
//       dut.io.start.poke(false.B)

//       // Write values into my_mem for addresses 0 to 25
//       for (addr <- 0 until 25) {
//         dut.io.wr_addr.poke(addr.U)
//         dut.io.wr_data.poke((addr * 2).U) // Example data pattern: double the address value
//         dut.io.wr_en.poke(true.B)
//         dut.clock.step(1)
//         dut.io.wr_en.poke(false.B)
//       }

//       // Verify values were written (optional, but useful for debugging)
//       for (addr <- 0 until 25) {
//         dut.io.rd_addr.poke(addr.U)
//         dut.io.rd_en.poke(true.B)
//         dut.clock.step(1)
//         val rdData = dut.io.rd_data.peek().litValue
//         val rdValid = dut.io.rd_data_valid.peek().litToBoolean
//         println(s"Memory[$addr] = $rdData")
//         dut.io.rd_en.poke(false.B)
//       }

//       // Start the operation
//       dut.io.init.poke(false.B)
...
//         // Access exposed signals
//         val rdAddr = dut.exposed_rd_addr.peek().litValue
//         val wrAddr = dut.exposed_wr_addr.peek().litValue
//         val oData = dut.exposed_o_data.peek().litValue
//         val iData = dut.exposed_i_data.peek().litValue
//         val done = dut.exposed_done.peek().litToBoolean

//         // Print the values for debugging
//         println(s"Cycle: rdAddr=$rdAddr, wrAddr=$wrAddr, oData=$oData, iData=$iData, done=$done")
...
//       fail("Operation did not complete within the expected cycles")
//     }
//   }
// }